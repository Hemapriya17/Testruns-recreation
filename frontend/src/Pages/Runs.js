...
export default function Runs() {
  ...
  const fetchRuns = () => {
    axios
      ...
      .then((response) => {
        const runsWithId = response.data.map((run) => ({
          ...run,
          id: run._id,
        }));
        const sortedRuns = runsWithId.sort(
  ...
  const fetchProcedures = () => {
    axios
      .get("http://localhost:8000/api/procedures")
      .then((response) => {
  ...
  const handleProcedureChange = (e) => {
    const selectedProcedure = procedures.find(
      (proc) => proc.procedureName === e.target.value
    );
  ...
  const handleSubmit = () => {
    if (editMode) {
      ...
      axios
        .put(`http://localhost:8000/api/runs/${selectedRunId}`, formData)
        .then((response) => {
      ...
      axios
        .post("http://localhost:8000/api/runs", {
          ...
          assignedBy: currentUser
            ? currentUser.displayName || "Unknown"
            : "Unknown", // Use the current user's display name
        })
        .then((response) => {
          fetchRuns(); // Refresh the list of runs
          handleClose(); // Close the form or modal
          // Navigate to the new runs page with the procedureID
          navigate(`/newruns/${response.data.procedureID}`); // Use response data to get procedureID
        })
        .catch((error) => {
  ...
  const handleDelete = () => {
    axios
      .delete(`http://localhost:8000/api/runs/${selectedRunId}`)
      .then(() => {
        fetchRuns();
        handleDeleteClose();
      })
