...
const Newprocedure = () => {
  ...
  useEffect(() => {
    ...
    if (location.state && location.state.procedure) {
      ...
      axios.get('http://localhost:8000/api/users/669b86da74088e9469ed9ac7') // Fetch user details if needed
        .then(response => {
  ...
  const handleSubmit = () => {
    ...
    const request = formData._id
      ? axios.put(`http://localhost:8000/api/procedures/${formData._id}`, formData)
      : axios.post('http://localhost:8000/api/procedures', formData);
